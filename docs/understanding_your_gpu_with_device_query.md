...
- Bank Conflict：共享内存被划分为 32 个 Bank。这已经讲烂了，总而言之就是一个 warp 线程访问共享内存时，多个线程不能同时访问同一个 bank 的数据，否则会变成串行的多次访问，共享内存的 bank id 归属是每 4 字节 (32bits) 顺序归属的，即连续的 0...31，0...31 这样
