...
（究极多干货预警，配图较多，涉及大量位运算与内存映射推导代码，建议在 PC 端阅读以获得最佳体验）
（本文适用于有一定 CUDA 编程基础，知道基本矩阵乘法原理，以及对手撕 kernel 性能调优感兴趣的读者阅读）
...
为什么直接选了BK=16？没有选8/32，有几点原因：

- 共享内存/寄存器资源够用情况下越大越好（可惜我暂时不能再大了），越大数据越连续，空间局部性越好，个人直觉更倾向大的；
- BK直接影响main loop的次数，选小一点比如8的话，会导致多一倍 循环控制 和 __syncthreads() 的开销；
- 后期改为双buffer可以让SM有足够多的compute指令时延去隐藏ldg时延。此外，虽然总的计算flop数不会变，但是单线程内循环展开的指令数变多，可以提高指令级并行度
- 如果改为 32，那么 smem 和寄存器用量会暴增，Occupancy 下降，甚至没法支持双 buffer。
